...
export async function puzzle2() {
	...
	rl.on('line', (line) => {
		...
		if (line.includes('Prize')) {
			const [x, y] = line.substring(7).split(',')
			price = {
				x: Number(x.split('=')[1]) + 10000000000000,
				y: Number(y.split('=')[1]) + 10000000000000,
			}
...
function findMachineSolution(machine: Machine): [number, number] {
	...
	const a =
		-(
			machine.price.y * machine.buttonB.x -
			machine.buttonB.y * machine.price.x
		) /
		(machine.buttonA.x * machine.buttonB.y -
			machine.buttonA.y * machine.buttonB.x)
	const b =
		(machine.buttonA.x * machine.price.y -
			machine.buttonA.y * machine.price.x) /
		(machine.buttonA.x * machine.buttonB.y -
			machine.buttonA.y * machine.buttonB.x)
